...
class cmb_Meta_Box {
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Löschen', self::$textdomain ),
					'defaultString' => __( 'Standard', self::$textdomain ),
					'pick'          => __( 'Farbe wählen', self::$textdomain ),
					'current'       => __( 'Aktuelle Farbe', self::$textdomain ),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'upload_file'     => __('Verwenden Sie diese Datei', self::$textdomain),
			'remove_image'    => __('Bild entfernen', self::$textdomain),
			'remove_file'     => __('Entfernen', self::$textdomain),
			'file'            => __('Datei:', self::$textdomain),
			'download'        => __('Herunterladen', self::$textdomain),
			'ajaxurl'         => admin_url( '/admin-ajax.php' ),
			'up_arrow'        => __('[ ↑ ]&nbsp;', self::$textdomain),
			'down_arrow'      => __('&nbsp;[ ↓ ]', self::$textdomain),
			'check_toggle'    => __( 'Alle aktivieren / deaktivieren', self::$textdomain ),
		) ) );
