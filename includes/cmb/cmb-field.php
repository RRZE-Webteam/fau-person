	...
	public function get_default_args() {
		return apply_filters(
			...
			array(
				...
				'data_delegate'       => null,
				'save_callback'       => null,
				'string-repeat-field' => __( 'Neu hinzufügen', CMB_Meta_Box::$textdomain ),
				'string-delete-field' => __( 'Feld entfernen', CMB_Meta_Box::$textdomain ),
			),
			get_class( $this )
		);
	}
...
class CMB_File_Field extends CMB_Field {
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'cmb-file-upload', plugins_url( 'cmb/js/file-upload.js', dirname( __FILE__ ) ), array( 'jquery', 'cmb-scripts' ) );
	...
	public function html() {
		...
		<div class="cmb-file-wrap" <?php echo 'data-type="' . esc_attr( $data_type ) . '"'; ?>>
			...
			<button class="button cmb-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>">
				<?php esc_html_e( 'Datei hinzufügen', CMB_Meta_Box::$textdomain ); ?>
			</button>
			...
			<button class="cmb-remove-file button <?php echo $this->get_value() ? '' : 'hidden'; ?>">
				<?php esc_html_e( 'Entfernen', CMB_Meta_Box::$textdomain ); ?>
			</button>
	...
	public function html() {
		...
		<div class="cmb-file-wrap" style="<?php echo esc_attr( $styles ); ?>" data-type="<?php echo esc_attr( $data_type ); ?>">
			...
			<button class="button cmb-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>" data-nonce="<?php echo wp_create_nonce( 'cmb-file-upload-nonce' ); ?>">
				<?php esc_html_e( 'Bild hinzufügen', CMB_Meta_Box::$textdomain ); ?>
			</button>
...
class CMB_Date_Field extends CMB_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'cmb-jquery-ui', plugins_url( 'cmb/css/vendor/jquery-ui/jquery-ui.css', dirname( __FILE__ ) ), '1.10.3' );

		wp_enqueue_script( 'cmb-datetime', plugins_url( 'cmb/js/field.datetime.js', dirname( __FILE__ ) ), array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'cmb-scripts' ) );
	}
...
class CMB_Time_Field extends CMB_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'cmb-timepicker', plugins_url( 'cmb/js/jquery.timePicker.min.js', dirname( __FILE__ ) ), array( 'jquery', 'cmb-scripts' ) );
		wp_enqueue_script( 'cmb-datetime', plugins_url( 'cmb/js/field.datetime.js', dirname( __FILE__ ) ), array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'cmb-scripts' ) );
	}
...
class CMB_Color_Picker extends CMB_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'cmb-colorpicker', plugins_url( 'cmb/js/field.colorpicker.js', dirname( __FILE__ ) ), array( 'jquery', 'wp-color-picker', 'cmb-scripts' ) );
...
class CMB_wysiwyg extends CMB_Field {
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'cmb-wysiwyg', plugins_url( 'cmb/js/field-wysiwyg.js', dirname( __FILE__ ) ), array( 'jquery', 'cmb-scripts' ) );
	}
...
class CMB_Select extends CMB_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'select2', plugins_url( 'cmb/js/vendor/select2/select2.js', dirname( __FILE__ ) ), array( 'jquery' ) );
		wp_enqueue_script( 'field-select', plugins_url( 'cmb/js/field.select.js', dirname( __FILE__ ) ), array( 'jquery', 'select2', 'cmb-scripts' ) );
	}
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'select2', plugins_url( 'cmb/js/vendor/select2/select2.css', dirname( __FILE__ ) ) );
	}
	...
	public function output_field() {
		...
		$none = isset($this->args['allow_none']) && is_string( $this->args['allow_none'] ) ? $this->args['allow_none'] : __( 'Keiner', CMB_Meta_Box::$textdomain );
	...
	public function output_script() {
		...
		$options = wp_parse_args( $this->args['select2_options'], array(
			'placeholder' => __( 'Suchbegriff eingeben', CMB_Meta_Box::$textdomain ),
			'allowClear'  => true,
		) );
...
class CMB_Group_Field extends CMB_Field {
	...
	function __construct() {
		...
		if ( ! empty( $this->args['fields'] ) ) {
			foreach ( $this->args['fields'] as $f ) {
				...
				$class = CMB_Helper::field_class_for_type( $f['type'] );
	...
	public function get_default_args() {
		return array_merge(
			...
			array(
				'fields'              => array(),
				'string-repeat-field' => __( 'Neue Gruppe hinzufügen', CMB_Meta_Box::$textdomain ),
				'string-delete-field' => __( 'Gruppe entfernen', CMB_Meta_Box::$textdomain ),
			)
		);
	}
...
class CMB_Gmap_Field extends CMB_Field {
	...
	public function get_default_args() {
		return array_merge(
			...
			array(
				...
				'default_zoom'        => '8',
				'string-marker-title' => __( 'Ziehen, um die genaue Position einzustellen.', CMB_Meta_Box::$textdomain ),
			)
		);
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'cmb-google-maps-script', plugins_url( 'cmb/js/field-gmap.js', dirname( __FILE__ ) ), array( 'jquery', 'cmb-google-maps' ) );
