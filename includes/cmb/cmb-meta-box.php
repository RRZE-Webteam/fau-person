...
class CMB_Meta_Box {
	...
	public function init_fields( $post_id = 0 ) {
		...
		foreach ( $this->_meta_box['fields'] as $key => $field ) {
			...
			$class = CMB_Helper::field_class_for_type( $field['type'] );
	...
	function enqueue_scripts() {
		...
		wp_enqueue_script( 'cmb-scripts', plugins_url( 'cmb/js/cmb.js', dirname( __FILE__ ) ), array( 'jquery' ) );

		wp_localize_script( 'cmb-scripts', 'CMBData', array(
			'strings' => array(
				'confirmDeleteField' => __( 'Sind Sie sicher, dass Sie dieses Feld löschen wollen?', self::$textdomain )
			)
		) );
	...
	function enqueue_styles() {
		...
		wp_enqueue_style( 'cmb-styles', plugins_url( 'cmb/css/dist/cmb.css', dirname( __FILE__ ) ) );
	...
	function save( $post_id = 0 ) {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( ! $class = CMB_Helper::field_class_for_type( $field['type'] ) ) {
